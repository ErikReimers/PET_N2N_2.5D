...
def main():
    ...
    model = get_model(model_name=args.model, slices=slices)
    ...
    if view == "a":
        ...
        all_noisy_data = np.zeros((w,h,nb_volumes))
    ...
    if view == "s":
        ...
        w = new_volume_shape[2]
        all_noisy_data = np.zeros((w,h,nb_volumes))
        all_denoised_data = np.zeros((w,h,nb_volumes))
        all_gt_data = np.zeros((w,h,nb_volumes))
    ...
    if view == "save":
        ...
        all_noisy_data = np.zeros((w,h,d,nb_volumes))
        all_denoised_data = np.zeros((w,h,d,nb_volumes))
        all_gt_data = np.zeros((w,h,d,nb_volumes))


    noise_volumes = np.zeros((new_volume_shape[1],new_volume_shape[2],new_volume_shape[0], nb_volumes))
    ground_truth_volumes = np.zeros((new_volume_shape[1],new_volume_shape[2],new_volume_shape[0], nb_volumes))
    ...
    for jj in range(nb_volumes):
        ...
        fid.close()
        noise_volumes[:,:,:,jj] = noise_volume[:,0:crop_x,0:crop_y].transpose((1, 2, 0))
        ...
        if args.ground_truth:
            ...
            fid.close()
            ground_truth_volumes[:,:,:,jj] = ground_truth_volume[:,0:crop_x,0:crop_y].transpose((1, 2, 0))
    ...
    if custom_max != 0:
        ...
        print("max value of all volumes: --custom_max ", np.max(noise_volumes))
        ground_truth_volumes = ground_truth_volumes/np.max(ground_truth_volumes)
        noise_volumes = noise_volumes/np.max(noise_volumes)
        

    #Run through each volume in the specified folder
    for jj in range(nb_volumes):

        #if testing an axial view
        if view == "a":

            noise_image = np.sqeeze(noise_volumes[:,:,slice_nb,jj])
            ground_truth_image = ground_truth_volumes[:,:,slice_nb,jj]
            slice_image = noise_volumes[:,:,slice_nb-slices//2:slice_nb+1+slices//2,jj]
            
            #If degrees were specified, rotate the noise image and the ground truth
            if degrees != 0:
                slice_image = imutils.rotate(slice_image,angle=degrees)
            ...
            denoised_image = np.squeeze(pred[0,:,:,slices//2])
        ...
        #If testing a sagittal view
            ...
            denoised_volume = np.zeros((new_volume_shape[1],new_volume_shape[2],new_volume_shape[0]))
            center_slice_nbs = range(slices//2,volume_shape[0]-slices//2)

            for ii in range(len(center_slice_nbs)):
                slice_image = noise_volumes[:,:,center_slice_nbs[ii]-slices//2:center_slice_nbs[ii]+1+slices//2,jj]
                pred = model.predict(np.expand_dims(slice_image, 0))
                denoised_volume[:,:,ii] = np.squeeze(pred[0,:,:,slices//2])
    ...
    for jj in range(all_noisy_data.shape[2]):
        if view != "save":
            ...
            out_image[:, :w] = ground_truth_image.transpose()
            out_image[:, w:w * 2] = noise_image.transpose()
            out_image[:, w * 2:] = denoised_image.transpose()
            ...
            if args.output_dir:
                ...
                example1 = np.repeat(np.repeat(np.squeeze(out_image[110:141,111:148]),15,axis=0),15,axis=1)
                plt.imsave(str(output_dir.joinpath(os.path.basename(volume_paths[jj])))[:-4] + "zoomGT.png", example1, vmin=0, vmax=1) 
                example2 = np.repeat(np.repeat(np.squeeze(out_image[110:141,(111+w):(148+h)]),15,axis=0),15,axis=1)
                plt.imsave(str(output_dir.joinpath(os.path.basename(volume_paths[jj])))[:-4] + "zoomN.png", example2, vmin=0, vmax=1)
                example3 = np.repeat(np.repeat(np.squeeze(out_image[110:141,(111+w*2):(148+h*2)]),15,axis=0),15,axis=1)
