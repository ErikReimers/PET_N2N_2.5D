...
def get_args():
    ...
    parser.add_argument("--volume_shape", nargs="*", type=int, default=[207,256,256],
                        help="What is the shape of the PET volumes")
...
def main():
    ...
    val_generator = ValGenerator(test_dir,gt_dir,nb_val_images = nb_val_images, rotations=rotations,sum_label=sum_label,volume_shape=volume_shape,slices=slices)
    ...
    if disp_examples == "True":
        ...
        for ii in range(3):
            ...
            axarr[0,0].imshow(np.squeeze(xg[ii,:,:,0]), vmin=0, vmax=np.max(xg[ii,:,:,:]) if volume_shape[2] == 276 else np.max(xg[ii,:,:,:])*0.2)
            axarr[0,1].imshow(np.squeeze(yg[ii,:,:,0]), vmin=0, vmax=np.max(yg[ii,:,:,:]) if sum_label == "True" or volume_shape[2] == 276 else np.max(yg[ii,:,:,:])*0.2)
            axarr[1,0].imshow(np.squeeze(xv[0,:,:,0]), vmin=0, vmax=np.max(xv) if volume_shape[2] == 276 else np.max(xv)*0.2)
            axarr[1,1].imshow(np.squeeze(yv[0,:,:,0]), vmin=0, vmax=np.max(yv) if sum_label == "True" or gt_dir != "not_specified" or volume_shape[2] == 276 else np.max(yv)*0.2)
